...
# --- CSS Styling (Sidebar Text Black) ---
st.markdown(
    """
    <style>
    ...
    .filter-container { background-color: #ffffff; padding: 15px; border-radius: 10px; margin-bottom: 20px; border: 1px solid #d1d3d4; }
    </style>
    """,
    ...
    with st.sidebar.form("assign_form", clear_on_submit=True):
        ...
        if st.form_submit_button("Assign Task Now"):
            ...
            if final_c:
                ...
                supabase.table("tasks").insert({"client_name": final_c, "work_type": work, "assigned_to": stf, "deadline": str(due), "status": "Pending", "file_url": f_url, "audio_url": a_url}).execute()
                st.sidebar.success("✅ Assigned!"); time.sleep(1); st.rerun()
...
if authenticated:
    if st.session_state.selected_task is not None:
        ...
        with st.container(border=True):
            ...
            m1, m2 = st.columns(2)
            with m1:
                # എറർ വരാതിരിക്കാൻ ഓഡിയോ ചെക്കിംഗ് മാറ്റം വരുത്തി
                a_url = task.get('audio_url')
                if a_url and isinstance(a_url, str) and a_url.startswith("http"):
                    st.write("**🎧 Audio Instruction:**")
                    st.audio(a_url)
            with m2:
        ...
        if tasks_data:
            ...
            if not is_admin: df = df[(df['assigned_to'] == current_user) & (df['status'] != 'Completed')]
            if not df.empty:
